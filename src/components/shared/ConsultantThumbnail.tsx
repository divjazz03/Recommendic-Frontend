...
const renderStats = (rating: number) => {
  return Array.from({ length: 5 }, (_, i) => (
    <Star
      key={i}
      className={`h-4 ${
        i < Math.floor(rating) ? "fill-main text-main" : "text-light-1"
      }`}
    />
  ));
};
...
const ConsultantThumbnail: React.FC<ConsultantThumbnailProps> = ({
  ...
  return (
    <div
      key={id}
      className="bg-white rounded-lg border-light-3 px-2 py-2 md:px-4 md:py-4 hover:shadow-md transition-shadow"
    >
        ...
        <div className="flex-1 space-y-3">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              <div className="text-lg font-semibold text-dark-4 ">
                $&nbsp;{fee}
              </div>
          ...
          <div className="flex items-start justify-start gap-2">
            <div className="flex items-center gap-1">{renderStats(rating)}</div>
          ...
          <div className="flex items-center gap-2 mb-3">
            {qualifications?.map((qual) => (
              <span
                key={qual}
                className="px-2 py-1 bg-main text-light-5 text-xs rounded-full"
              >
                {qual}
              </span>
            ))}
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              <div
                className={`px-1 sm:px-3 py-1 rounded-sm text-xs font-thin sm:text-sm ${getAvailabilityColor(
                  availability
                )}`}
              >
                {availability}
            ...
            <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm">
              <Link
                to={`profile/${id}`}
                className="px-2 py-2 sm:px-4 sm:py-2 text-main border border-main rounded-lg hover:bg-main/20 transition-colors"
              >
                View&nbsp;Profile
              </Link>
              <Link
                to={`/consultant/schedule/${id}`}
                className="px-2 py-2 sm:px-4 sm:py-2 bg-main text-white rounded-lg hover:bg-main transition-colors flex items-center gap-2"
              >
