...
const ConsultantScreen = () => {
  ...
  if (!param || !param.consultantId) {
    return (
      <Empty>
        <EmptyHeader>
          <EmptyTitle>Consultant Id Not Present</EmptyTitle>
          <EmptyDescription>
            ConsultantId in query param not present
          </EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
          <Button onClick={() => {}}>
  ...
  useEffect(() => {
    ...
    setConsultant({
      ...
      nextAvailable: returnedDataResponse?.availableSlots
        ? returnedDataResponse.availableSlots[0]?.dateTime
        : "",
            ...
            <div className="flex flex-row lg-flex-col items-start gap-6">
              <div className="relative items-start">
                ...
                <div
                  className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full flex items-center justify-center ${
                    consultant?.isVisible ? "bg-green-500" : "bg-gray-400"
                  }`}
                >
              ...
              <div className="flex-1 flex-row">
                <div className="flex items-center gap-3 mb-2">
                  <h1 className="text-2xl sm:text-3xl font-semibold sm:font-bold text-main">
                    {consultant?.name}
                  </h1>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      className="px-6 py-3 bg-main-light hover:bg-main text-white rounded-xl font-semibold transition-color duration-100 flex items-center gap-2"
                      onClick={() =>
                        navigate(`/consultant/schedule/${param.consultantId}`)
                      }
                    >
                    ...
                    <button
                      onClick={() =>
                        navigate("/chat", { state: { id: consultant?.id } })
                      }
          ...
          {[
            {
              ...
              value: consultant?.stats?.patientsHelped?.toLocaleString(),
              color: "blue",
            },
            {
              icon: TrendingUp,
              label: "Success Rate",
              value: `${consultant?.stats?.successRate}%`,
              color: "green",
            },
            {
              icon: Clock,
              label: "Response Time",
              value: consultant?.stats?.responseTime,
              color: "purple",
            },
            {
              icon: Heart,
              label: "Follow-up Rate",
              value: `${consultant?.stats?.followUpRate}%`,
              color: "red",
            },
          ].map((stat, index) => (
            <div
              key={index}
              className="bg-white rounded-2xl p-4 sm:p-6 hover:shadow-sm border transition-all duration-100"
            >
              <div
              ...
              <div className="text-2xl font-bold text-dark-3 mb-1">
          ...
          <div className="flex justify-around border-b border-light-3">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-2 py-2 sm:px-4 sm:py-4 font-medium transition-all duration-100 ${
                  activeTab === tab.id
              ...
              >
                <tab.icon
                  className={` h-5 w-5 ${
                    tab.id === activeTab ? "fill-main h-6 w-6 text-white" : ""
                  }`}
                />
                {tab.label}
              </button>
          ...
          <section className="">
            {activeTab === "overview" && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-xl font-semibold text-dark-3 mb-3">
            ...
            {activeTab === "availability" &&
              (!consultant?.availableSlots ? (
                <Empty>
                  <EmptyHeader>
                    <EmptyTitle>Not available today</EmptyTitle>
                    <EmptyDescription>
                      This Consultant will not be available anytime today
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
              ) : (
                <section className="space-y-6">
                  ...
                  <div className="grid gap-3">
                    {consultant?.availableSlots?.map((slot, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-light-1 hover:bg-grey-100 transition-all duration-300"
                      >
            ...
            {/*Reviews Tab */}
            {activeTab === "reviews" &&
              (!consultant?.reviews || consultant.reviews.length < 1 ? (
                <Empty>
                  <EmptyHeader>
                    <EmptyTitle>No reviews yet</EmptyTitle>
                    <EmptyDescription>
                      This Consultant has no reviews
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
              ) : (
                <section className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-dark-3">
                        {consultant?.rating}
                      </div>
                      <div className="text-sm text-dark-1">
                        {consultant?.totalReviews} reviews
                        ...
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-main-light rounded-full flex items-center justify-center">
                              <span className="text-light-4 font-medium">
                          ...
                          <div className="flex">
                            {[...Array(review.rating)].map((_, i) => (
                              <Star
                                key={i}
                                className="w-4 h-4 text-main fill-current"
                              />
                            ))}
                          </div>
                        </div>
