...
export const usePatientSchedule = (consultantId: string) => {
  ...
  const { data, isPending } = useGetConsultantSchedules(
    consultantId,
    selectedDate.toISOString().split("T")[0],
    accessToken,
  );
  const [consultationType, setConsultationType] =
    useState<ConsultationChannel>("in_person");
  const [currentStep, setCurrentStep] = useState(1);
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [consultantScheduleData, setConsultantScheduleData] =
    useState<ConsultantScheduleData>();
  const callBackSchedulesToTimeSlots = useCallback(schedulesToTimeSlots, [
    data?.data,
    selectedDate,
  ]);
  const { mutateAsync: createAnAppointment, isPending: isCreating } =
    useCreateAppointment(accessToken);
  ...
  const isTimeSlotAvailable = (time: string): boolean => {
    return (
      !consultantScheduleData?.timeSlots.unavailableSlots
        .map((slot) => slot.dateTime)
        .includes(time) &&
      !consultantScheduleData?.timeSlots.bookedSlots
        .map((slot) => slot.dateTime)
        .includes(time)
    );
  };
...
export const schedulesToTimeSlots = (schedules: TimeSlot[]): TimeSlots => {
  ...
  schedules?.forEach((schedule) => {
    ...
    const formattedTime: string = DateTime.fromISO(schedule.dateTime, {
      zone: "utc",
    }).toFormat("HH:mm");
    ...
    const scheduleTimeIsMorning =
      scheduleTime >= morningTime && scheduleTime < afternoonTime;
    const scheduleTimeIsAfternoon =
      scheduleTime >= afternoonTime && scheduleTime < eveningTime;
