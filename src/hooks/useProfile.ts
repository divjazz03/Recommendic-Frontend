...
export interface ModifyingProfileData {
    firstName?: string
    lastName?: string
    phoneNumber?: string
    dateOfBirth?: string
...
export const usePatientProfile = () => {
    ...
    const [modifyingProfileData, setModifyingProfileData] = useState<ModifyingProfileData>();

    let selectedCategoriesChangable = myProfileResponse?.data.interests ?? []

    const [selectedCategories, setSelectedCategories] = useState<MedicalCategory[]>(
        medicalCategoriesResponse?.data.filter(category => selectedCategoriesChangable.includes(category.name))??[]
    )
    
    const {mutateAsync: updatePatientProfileData, isPending: isUpdating} = useUpdatePatientData();
    ...
    const handleAddressChange = (field: keyof Address, value: string) => {
        ...
        if (modifyingProfileData){
            setModifyingProfileData({...modifyingProfileData,
        address: {...profileData?.address, [field]: value}});
        } else {
            setModifyingProfileData(prev => ({...prev, address:{[field]:value}}));
        }
...
export const useConsultantProfile = () => {
    ...
    const [isEditing, setIsEditing] = useState(false);
    const [showSuccessMessage, setShowSuccessMessage] = useState(false);
    const [modifyingProfileData, setModifyingProfileData] = useState<ModifyingConsultantProfileData>();
    const [profileData, setProfileData] = useState<ConsultantProfileData>();

    const handleInputChange = (field: keyof ModifyingConsultantProfileData, value: unknown) => {
        setProfileData({ ...profileData, [field]: value });
        setModifyingProfileData({ ...modifyingProfileData, [field]: value })
    };

    const handleEducationChange = (field: keyof ConsultantEducation, value: unknown) => {
        setProfileData({...profileData, education:{...profileData?.education, [field]: value}})
        setModifyingProfileData({...modifyingProfileData, education: {...modifyingProfileData?.education, [field]: value}})
    }
