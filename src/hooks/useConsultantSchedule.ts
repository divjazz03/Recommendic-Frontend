...
import {
  useCreateNewSchedules,
  useDeleteSchedule,
  useGetCurrentUserSchedules,
  useGetScheduleWithUserId,
  useUpdateSchedule,
...
import {
  ModifyingSchedule,
  ModifyingRecurrenceRule,
} from "@/components/consultant/ConsultantModifySchedule";
...
export const useCreateSchedule = () => {
  ...
  const { mutateAsync: createNewSchedules, isPending: isCreating } =
    useCreateNewSchedules(accessToken);
      ...
      schedules.map((schedule) => {
        ...
        if (schedule.recurrenceRule && schedule.recurrenceRule.weekDays) {
          schedule.recurrenceRule.weekDays =
            schedule.recurrenceRule.weekDays?.includes(day)
  ...
  const createSchedules = async () => {
    ...
    try {
      ...
      return toast.error(
        `Could not create schedule - ${apiError.message}:  ${apiError?.data}`,
      );
    }
  };
...
export const useModifySchedule = (location: Location) => {
  ...
  const { data: scheduleResponse } = useGetScheduleWithUserId(
    scheduleId,
    accessToken,
  );
  const { mutateAsync: updateAsyncSchedule } = useUpdateSchedule(accessToken);
  const { mutateAsync: deleteAsyncSchedule } = useDeleteSchedule(accessToken);
  const [modificationSuccess, setModificationSuccess] =
  ...
  const updateRecurrenceRule = (
    field: keyof ModifyingRecurrenceRule,
    value: unknown,
  ) => {
    setModifiedSchedule((schedule) => ({
      ...schedule,
      recurrenceRule: { ...schedule?.recurrenceRule, [field]: value },
    }));
  };
      ...
      setModifiedSchedule((schedule) => {
        if (day) {
          if (modifiedSchedule.recurrenceRule) {
            const weekDays =
              modifiedSchedule?.recurrenceRule?.weekDays?.includes(day)
                ? modifiedSchedule?.recurrenceRule?.weekDays?.filter(
                    (d) => d !== day,
                  )
                : schedule?.recurrenceRule?.weekDays
                  ? [...schedule.recurrenceRule?.weekDays, day]
...
export const formatRecurrence = (rule: RecurrenceRule) => {
  if (rule.frequency === "weekly") {
    const interval =
      rule.interval > 1 ? `every ${rule.interval} weeks` : "weekly";
...
export const useScheduleDisplay = () => {
  ...
  const { data: schedulesResponse, isPending: isUserSchedulesLoading } =
    useGetCurrentUserSchedules(accessToken);
