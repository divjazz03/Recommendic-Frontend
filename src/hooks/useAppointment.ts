...
const getDaysUntil = (dateStr: string) => {
  ...
  const diffTime = DateTime.fromJSDate(appointmentDate).diffNow("days", {
...
export const usePatientAppointment = () => {
  ...
  const [selectedAppointment, setSelectedAppointment] =
    useState<PatientAppointmentType>();
  ...
  const upcomingAppointmentCount = appointments?.filter(
    (apt) =>
      new Date(`${apt.date}T${apt.time}`) >= new Date() &&
      apt.status !== "cancelled" &&
      apt.status !== "completed"
  ...
  const pastAppointments = filteredAppointments?.filter((apt) => {
    return (
      Date.parse(`${apt.date}T${apt.time}`) > Date.now() ||
      apt.status === "completed" ||
      apt.status === "cancelled"
    );
  });
  ...
  useEffect(() => {
    const localAppointments = appointmentsResponse?.data
      .content as PatientAppointmentType[];
...
export const useConsultantAppointment = () => {
  ...
  useEffect(() => {
    const localAppointments =
      (appointmentsResponse?.data.content as ConsultantAppointmentType[]) || [];
    setAppointments([...localAppointments]);
  }, [appointmentsResponse]);
